...
class UNet(object):
    ...
    def __init__(self, batch_size, img_height, img_width, learning_rate, sess):
        ...
        self.flattened_pred = tf.reshape(self.prediction, [-1])
    ...
    def build_network(self, input_tensor, initializer, is_training, num_classes=14):
        ...
        conv_layer_enc_64 = resnet_utils.conv_bn_layer(conv_layer, name='encoder', kernel_size=(3,3),
        ...
        conv_layer_enc_128 = resnet_utils.conv_bn_layer(conv_layer, name='encoder', kernel_size=(3, 3),
                                                   output_channels=128, initializer=initializer,
                                                   stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_enc_128 = resnet_utils.conv_bn_layer(conv_layer_enc_128, name='encoder', kernel_size=(3, 3),
                                                   output_channels=128, initializer=initializer,
                                                   stride=1, bn=True, is_training=is_training, relu=True)
        ...
        conv_layer_enc_256 = resnet_utils.conv_bn_layer(conv_layer, name='encoder', kernel_size=(3, 3),
                                                    output_channels=256, initializer=initializer,
                                                    stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_enc_256 = resnet_utils.conv_bn_layer(conv_layer_enc_256, name='encoder', kernel_size=(3, 3),
                                                    output_channels=256, initializer=initializer,
                                                    stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer = layers.max_pool2d(inputs=conv_layer_enc_256, kernel_size=(2, 2), stride=2)

        # Input size is H/8 x W/8
        conv_layer_enc_512 = resnet_utils.conv_bn_layer(conv_layer, name='encoder', kernel_size=(3, 3),
                                                    output_channels=512, initializer=initializer,
                                                    stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_enc_512 = resnet_utils.conv_bn_layer(conv_layer_enc_512, name='encoder', kernel_size=(3, 3),
                                                    output_channels=512, initializer=initializer,
                                                    stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer = layers.max_pool2d(inputs=conv_layer_enc_512, kernel_size=(2, 2), stride=2)


        conv_layer_enc_1024 = resnet_utils.conv_bn_layer(conv_layer, name='encoder', kernel_size=(3, 3),
                                                    output_channels=1024, initializer=initializer,
                                                    stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_dec_512 = resnet_utils.conv_bn_layer(conv_layer_enc_1024, name='dencoder', kernel_size=(3, 3),
                                                    output_channels=512, initializer=initializer,
                                                    stride=1, bn=True, is_training=is_training, relu=True)

        conv_layer_dec_1024=  tf.concat([conv_layer_dec_512, conv_layer_enc_512], axis=3)
        conv_layer_dec_512 = resnet_utils.conv_bn_layer(conv_layer_dec_1024, name='dencoder', kernel_size=(3, 3),
                                                        output_channels=512, initializer=initializer,
                                                        stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_dec_256 = resnet_utils.conv_bn_layer(conv_layer_dec_512, name='dencoder', kernel_size=(3, 3),
                                                        output_channels=256, initializer=initializer,
                                                        stride=1, bn=True, is_training=is_training, relu=True)
        ...
        conv_layer_dec_256 = resnet_utils.conv_bn_layer(conv_layer_dec_512, name='dencoder', kernel_size=(3, 3),
                                                        output_channels=256, initializer=initializer,
                                                        stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_dec_128 = resnet_utils.conv_bn_layer(conv_layer_dec_256, name='dencoder', kernel_size=(3, 3),
                                                        output_channels=128, initializer=initializer,
                                                        stride=1, bn=True, is_training=is_training, relu=True)
        ...
        conv_layer_dec_128 = resnet_utils.conv_bn_layer(conv_layer_dec_256, name='dencoder', kernel_size=(3, 3),
                                                        output_channels=128, initializer=initializer,
                                                        stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_dec_64 = resnet_utils.conv_bn_layer(conv_layer_dec_128, name='dencoder', kernel_size=(3, 3),
                                                        output_channels=64, initializer=initializer,
                                                        stride=1, bn=True, is_training=is_training, relu=True)
        ...
        conv_layer_dec_64 = resnet_utils.conv_bn_layer(conv_layer_dec_128, name='dencoder', kernel_size=(3, 3),
                                                        output_channels=64, initializer=initializer,
                                                        stride=1, bn=True, is_training=is_training, relu=True)
        conv_layer_dec_64 = resnet_utils.conv_bn_layer(conv_layer_dec_64, name='dencoder', kernel_size=(3, 3),
                                                       output_channels=64, initializer=initializer,
                                                       stride=1, bn=True, is_training=is_training, relu=True)
